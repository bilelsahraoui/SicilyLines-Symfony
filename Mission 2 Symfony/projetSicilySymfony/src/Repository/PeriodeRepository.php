...
class PeriodeRepository extends ServiceEntityRepository
{
    ...
    */
    ...
    public function getPeriode($val){
        $req=$this->getEntityManager()->createQuery("select p from App\Entity\Periode p 
        where '" . $val->format('Y-m-d') . "' <= p.dateFin 
        and '" . $val->format('Y-m-d') . "' >= p.dateDebut");
